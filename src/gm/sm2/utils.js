...
function generatePublicKeyFromPrivateKey(privateKey, compressed = true) {
...
function getRawPublicKeyFromCompressedPubicKey(compressed_pub) {
    let prefix = compressed_pub.substr(0,2);
    if (prefix !== '02' && prefix !== '03') {
        throw new Error(`${compressed_pub} is not a valid compressed public key.`);
    }

    let P = getGlobalCurve().decodePointHex(compressed_pub);
    let Px = leftPad(P.getX().toBigInteger().toString(16), 64);
    let Py = leftPad(P.getY().toBigInteger().toString(16), 64);
    let publicKey = '04' + Px + Py;
    return publicKey;
...
module.exports = {
    ...
    getRawPublicKeyFromCompressedPubicKey,
